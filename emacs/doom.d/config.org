...
* Improve boot up speed :noexport:

** Omit default startup screen :noexport:
...
** Disable =package-enable-at-startup= :noexport:
...
** Unset =file-name-handler-alist= :noexport:
...
** Disable =site-run-file= :noexoprt:

#+BEGIN_SRC emacs-lisp :tangle no
(setq site-run-file nil)
#+END_SRC

** Disable Unnecessary Interface :noexport:
...
** Setting up garbage collection for speedy startup :noexoprt:
...
*** Better garbage threshold limit :noexoprt:
...
*** Garbage collect when Emacs is out of focus :noexoprt:
